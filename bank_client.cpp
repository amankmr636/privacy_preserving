...
int main(int argc, char** argv) {
  ...
  for (std::size_t k = 0; k < K; ++k) {
    indSlots[k] = DecryptSlots(artifacts.cc, artifacts.skBank, (*finalIndicators)[k],
                               nUsers);
  }
  ...
  auto fraudScores =
      DecryptSlots(artifacts.cc, artifacts.skBank, (*finalFraudScores)[0], nUsers);
  ...
  auto wcss = DecryptSlots(artifacts.cc, artifacts.skBank, (*finalMetrics)[0], 1)[0];
  auto silhouette =
      DecryptSlots(artifacts.cc, artifacts.skBank, (*finalMetrics)[1], 1)[0];
  ...
  for (std::size_t idx = 0; idx < (*finalCentroids).size(); ++idx) {
    auto value =
        DecryptSlots(artifacts.cc, artifacts.skBank, (*finalCentroids)[idx], 1)[0];
