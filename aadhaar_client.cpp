...
int main(int argc, char** argv) {
  ...
  auto data = Normalize01(LoadCsvNumeric("data/aadhaar.csv"));
  AddDpNoise(data, 0.002);
  const std::size_t nUsers = data.size();

  int sock = Connect(host, kPortAggregator);
  ...
  for (std::size_t iter = 0; iter < maxIters; ++iter) {
    ...
    auto fraud = recvCipherVector(sock);
    if (!indicators || !metrics || !fraud)
  ...
  // Final encrypted payloads from aggregator for client-side decryption/export.
  auto indicators = recvCipherVector(sock);
  auto metrics = recvCipherVector(sock);
  auto fraudPayload = recvCipherVector(sock);
  auto encCentroids = recvCipherVector(sock);
  if (!indicators || !metrics || !fraudPayload || !encCentroids)
  ...
  for (std::size_t k = 0; k < K; ++k) {
    indSlots[k] =
        DecryptSlots(artifacts.cc, artifacts.skAadhaar, (*indicators)[k], nUsers);
  }
  ...
  // fraud_scores.csv and fraud_rankings.csv (ranking by explicit sort).
  auto fraudScores =
      DecryptSlots(artifacts.cc, artifacts.skAadhaar, (*fraudPayload)[0], nUsers);
  ...
  // metrics.csv
  auto wcss = DecryptSlots(artifacts.cc, artifacts.skAadhaar, (*metrics)[0], 1)[0];
  auto silhouette =
      DecryptSlots(artifacts.cc, artifacts.skAadhaar, (*metrics)[1], 1)[0];
  ...
  for (std::size_t idx = 0; idx < (*encCentroids).size(); ++idx) {
    auto value =
        DecryptSlots(artifacts.cc, artifacts.skAadhaar, (*encCentroids)[idx], 1)[0];
